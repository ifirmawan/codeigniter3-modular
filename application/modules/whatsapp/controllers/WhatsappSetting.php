...
class WhatsappSetting extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->library('Whatsapp_manager');
    ...
    public function whatsapp_api()
    {
        ...
        if ($page == '') {
            $this->session->set_userdata('WhatsAppAPILastSerial', "");
        } else {
            $per_page = $this->input->get_post('per_page');
            $start = ($page-1) * $per_page;
            $this->session->set_userdata('WhatsAppAPILastSerial', $start);
        }
        ...
        $crud->where('whatsapp_api_config.deleted', '0');
        $crud->where('whatsapp_api_config.user_id', $this->user_id);

        $crud->set_theme('flexigrid');
        $crud->set_table('whatsapp_api_config');
        $crud->order_by('gateway_name');
        $crud->set_subject($this->lang->line('WhatsApp API'));
        ...
        $crud->callback_after_insert(array($this, 'insert_user_id_whatsapp_api'));    /**insert the user_id***/

        // for SL column
        $crud->callback_column('SL', array($this, 'generateSerialWhatsAppAPI'));
    ...
    public function whatsapp_template()
    {
        ...
        $page = $this->input->get_post('page');
        if ($page == '') {
            $this->session->set_userdata('WhatsAppTemplateLastSerial', "");
        } else {
            $per_page = $this->input->get_post('per_page');
            $start = ($page-1) * $per_page;
            $this->session->set_userdata('WhatsAppTemplateLastSerial', $start);
        }
        ...
        $crud->set_subject($this->lang->line('WhatsApp Template'));
        $crud->required_fields('message', 'template_name');
        $crud->columns('SL', 'template_name', 'message');
        $crud->fields('template_name', 'message');

        $crud->callback_after_insert(array($this, 'insert_user_id_whatsapp_template'));    /**insert the user_id***/

        // for SL column
        $crud->callback_column('SL', array($this, 'generateSerialWhatsAppTemplate'));
        ...
        $output = $crud->render();
        $data['output']=$output;
        $data['page_title'] = 'WhatsApp Template';
        $data['crud']=1;
        $this->_viewcontroller($data);
    }
    ...
    {
        $data['body']       = "my_whatsapp/schedule_whatsapp/scheduled_whatsapp";
        $data['page_title'] = $this->lang->line('WhatsApp Campaign');
        $this->_viewcontroller($data);
    }

    public function whatsapp_campaign_data()
    {
        ...
        if($is_searched)
        {
            $this->session->set_userdata('whatsapp_sending_campaign_name', $campaign_name);
            $this->session->set_userdata('whatsapp_sending_posting_status', $posting_status);
            $this->session->set_userdata('whatsapp_sending_scheduled_from', $scheduled_from);
            $this->session->set_userdata('whatsapp_sending_scheduled_to', $scheduled_to);
        }

        $search_campaign_name   = $this->session->userdata('whatsapp_sending_campaign_name');
        $search_posting_status  = $this->session->userdata('whatsapp_sending_posting_status');
        $search_scheduled_from  = $this->session->userdata('whatsapp_sending_scheduled_from');
        $search_scheduled_to    = $this->session->userdata('whatsapp_sending_scheduled_to');
        ...
        $where_simple['whatsapp_sending_campaign.user_id'] = $this->user_id;

        $where  = array('where' => $where_simple);
        $join   = array("whatsapp_api_config" => "whatsapp_sending_campaign.api_id=whatsapp_api_config.id,left");
        $select = array("whatsapp_sending_campaign.*","CONCAT(whatsapp_api_config.gateway_name,' : ',whatsapp_api_config.phone_number) AS send_as");
        $offset = ($page-1)*$rows;
        $result = array();
        $info   = $this->basic->get_data('whatsapp_sending_campaign', $where, $select, $join, $limit=$rows, $start=$offset, $order_by=$order_by_str, $group_by='', $num_rows=0);
        for($i = 0; $i < count($info); $i++)
        {
            ...
            if($this->config->item("number_of_whatsapp_to_be_sent_in_try") == "" || $this->config->item("number_of_whatsapp_to_be_sent_in_try") == "0")
            {
                $force_porcess_str="";
            }
            else
            ...
            {
                $edit_url = site_url('my_whatsapp/edit_whatsapp_campaign/'.$info[$i]['id']);
        ...
        $total_rows_array = $this->basic->count_row($table="whatsapp_sending_campaign", $where, $count="whatsapp_sending_campaign.id", $join);
    ...
    {
        $data['body']="my_whatsapp/schedule_whatsapp/schedule_add";
        ...
        $data['whatsapp_template'] = $this->basic->get_data('message_template', $where, $select=array("id","template_name"), $join='', $limit='', $start='', $order_by='template_name ASC', $group_by='', $num_rows=0);

        $where_simple = array();
        $temp_userid = $this->user_id;

        /***get whatsapp config***/
        if($this->config->item('whatsapp_api_access') == '1' && $this->session->userdata("user_type") == 'User')
        {
            $join = array('users' => 'whatsapp_api_config.user_id=users.id,left');
            $select = array('whatsapp_api_config.*','users.id AS usersId','users.user_type');
            $where_in = array('whatsapp_api_config.user_id'=>array('1',$temp_userid),'users.user_type'=>array('Admin','User'));
            $where = array('where_simple'=> array('status'=>'1'),'where_in'=>$where_in);
            $whatsapp_api_config=$this->basic->get_data('whatsapp_api_config', $where, $select, $join, $limit='', $start='', $order_by='phone_number ASC', $group_by='', $num_rows=0);
        } else
        {
        ...
        $whatsapp_api_config_option=array();
        foreach ($whatsapp_api_config as $info) {
            $id=$info['id'];
            $whatsapp_api_config_option[$id]=$info['gateway_name'].": ".$info['phone_number'];
        }

        $data['whatsapp_option'] = $whatsapp_api_config_option;
        $data['groups_name']  = isset($group_name) ? $group_name: "";
        $data["time_zone"]     = $this->_time_zone_list();
        $data['page_title'] = $this->lang->line('Add WhatsApp Campaign');
        $this->_viewcontroller($data);
    }
    ...
    public function add_schedule_action()
    {
        ...
        $whatsapp_api       = strip_tags(trim($this->input->post('from_whatsapp', true)));
        ...
        $contacts_whatsapp_group = $this->input->post('contacts_id', true);
        if(isset($contacts_whatsapp_group) && !empty($contacts_whatsapp_group))
            $contact_groupid    = implode(",",$contacts_whatsapp_group);
        ...
        if(isset($contacts_whatsapp_group) && !empty($contacts_whatsapp_group))
        foreach ($contacts_whatsapp_group as $key => $value)
        {
            $where_simple = array('contacts.user_id'=>$this->user_id);
        ...
        // inserting data of whatsapp_campaign_campaign Table
        $inserted_data = array(
            "user_id"           => $this->user_id,
            "api_id"            => $whatsapp_api,
        ...
        if($this->basic->insert_data("whatsapp_sending_campaign", $inserted_data))
        {
            ...
            foreach ($report as $key=>$value)
            {
                ...
                $this->basic->insert_data("whatsapp_sending_campaign_send", $report_insert);
            }
    ...
    public function campaign_sent_status()
    {
        ...
        $campaign_data     = $this->basic->get_data("whatsapp_sending_campaign",array("where"=>array("id"=>$id,"user_id"=>$this->user_id)));
        ...
        // edit message url
        $edit_url = site_url('my_whatsapp/edit_message_content/'.$campaign_data[0]['id']);
        ...
        // checking posting status from whatsapp_sending_campaign table
        if( $posting_status == '2')
    ...
    public function edit_whatsapp_campaign($id=0)
    {
        if($id==0) exit();

        $data['body']          = "my_whatsapp/schedule_whatsapp/edit_whatsapp_campaign";
        $data["time_zone"]     = $this->_time_zone_list();
        $data["campaign_data"] = $this->basic->get_data("whatsapp_sending_campaign",array("where"=>array("id"=>$id,"user_id"=>$this->user_id)));
        ...
        if($this->config->item('whatsapp_api_access') == '1' && $this->session->userdata("user_type") == 'User')
        {
            $join = array('users' => 'whatsapp_api_config.user_id=users.id,left');
            $select = array('whatsapp_api_config.*','users.id AS usersId','users.user_type');
            $where_in = array('whatsapp_api_config.user_id'=>array('1',$this->user_id),'users.user_type'=>array('Admin','User'));
            $where = array('where_simple'=> array('status'=>'1'),'where_in'=>$where_in);
            $whatsapp_api_config=$this->basic->get_data('whatsapp_api_config', $where, $select, $join, $limit='', $start='', $order_by='phone_number ASC', $group_by='', $num_rows=0);
        } else
        {
        ...
        $data['whatsapp_option']=$whatsapp_api_config_option;
        $data['groups_name']   = isset($group_name) ? $group_name: "";
        $data['page_title']    = $this->lang->line('Edit WhatsApp Campaign');


        $this->_viewcontroller($data);
    }

    public function edit_whatsapp_campaign_action()
    {
        if(!$_POST) exit();
        ...
        // updating data of whatsapp_campaign_campaign Table
        $updated_data = array(
            "user_id"           => $this->user_id,
            "api_id"            => $whatsapp_api,
            "contact_ids"       => isset($contacts_id) ? $contacts_id:"",
        ...
        /* updating whatsapp_sending_campaign table data of the campaign */
        if($this->basic->update_data("whatsapp_sending_campaign", array("id" => $campaign_id,"user_id"=>$this->user_id), $updated_data))
        {
            /* Delete the rows of updated campaign from whatsapp_sending_campaign_send table */
            $this->basic->delete_data("whatsapp_sending_campaign_send", array("campaign_id" =>$campaign_id));
            ...
            foreach ($report as $key=>$value)
            {
                ...
                /* Inserting again the updated report data into whatsapp_sending_campaign_send table */
                $this->basic->insert_data("whatsapp_sending_campaign_send", $report_insert);
            }
    ...
    public function edit_message_content($id=0)
    {
        ...
        $data['body'] = "my_whatsapp/schedule_whatsapp/edit_message_content";
        $data['page_title'] = $this->lang->line("Edit Message");
        $data["xdata"] = $this->basic->get_data("whatsapp_sending_campaign",array("where"=>array("id"=>$id,"user_id"=>$this->user_id)));
        $this->_viewcontroller($data);
    }
    ...
    public function edit_message_content_action()
    {
        ...
        $this->basic->update_data('whatsapp_sending_campaign',array("id"=>$campaign_id,"user_id"=>$this->user_id),$edited_message);
    }
    ...
    public function force_reprocess_campaign()
    {
        ...
        $this->basic->update_data('whatsapp_sending_campaign',$where,$data);
    ...
    public function ajax_campaign_pause()
    {
        ...
        $post_info = $this->basic->update_data('whatsapp_sending_campaign',array('id'=>$table_id),array('posting_status'=>'3','is_try_again'=>'0'));
    ...
    {
        $table_id  = $this->input->post('table_id');
        $post_info = $this->basic->update_data('whatsapp_sending_campaign',array('id'=>$table_id),array('posting_status'=>'1','is_try_again'=>'1'));
    ...
    public function delete_campaign()
    {
        ...
        if($this->basic->delete_data("whatsapp_sending_campaign",array("id"=>$id,"user_id"=>$this->user_id)))
        {
            if($this->basic->delete_data("whatsapp_sending_campaign_send",array("campaign_id"=>$id,"user_id"=>$this->user_id)))
            echo "1";
        }
        else echo "0";
    }
    // end whatsapp sending functions
    ...
    public function schedule_contacts()
    {
        ...
        $this->load->view('my_whatsapp/schedule_whatsapp/schedule_contact_details', $data);
    }
    ...
    public function delete_schedule($id=0)
    {
        ...
        if($this->basic->delete_data('schedule_whatsapp', $where))
        {
            $this->session->set_flashdata('delete_success_message', 1);
            redirect('my_whatsapp/scheduled_whatsapp', 'location');
        }
    }

    public function birthday_whatsapp()
    {
        //This process is important to reset the last serial no...
        $page = $this->input->get_post('page');
        if ($page == '') {
            $this->session->set_userdata('birthdayWhatsAppLastSerial', "");
        } else {
            $per_page = $this->input->get_post('per_page');
            $start = ($page-1) * $per_page;
            $this->session->set_userdata('birthdayWhatsAppLastSerial', $start);
        }
        ...
        $crud->order_by('id');
        $crud->set_subject($this->lang->line('Scheduled WhatsApp (Birthday Wish)'));
        ...
        // for SL column
        $crud->callback_column('SL', array($this, 'generateSerialBirhdayWhatsApp'));
        ...
        if ($state == 'add' || $state=='edit') {
            ...
            $crud->callback_field('whatsapp_template', array($this, 'whatsapp_template_field'));
            $crud->fields('api_id', 'whatsapp_template', 'message', 'time_zone', 'status');
        }
        ...
        $crud->display_as('whatsapp_template', $this->lang->line('WhatsApp Template'));
        ...
        $output = $crud->render();
        $data['page_title'] = 'Birthday Wish WhatsApp';
        $data['output']=$output;
        $data['crud']=1;
        $this->_viewcontroller($data);
    }
    ...
    {
        $data['body']="my_whatsapp/whatsapp_history/whatsapp_history";
        $data['page_title'] = 'My WhatsApp History';
        $this->_viewcontroller($data);
    }


    public function my_whatsapp_history_data()
    {
        $page = isset($_POST['page']) ? intval($_POST['page']) : 1;
        $rows = isset($_POST['rows']) ? intval($_POST['rows']) : 5;
        ...
        $where_simple['whatsapp_history.user_id']=$this->user_id;
        $where=array('where'=>$where_simple);
        $select=array("whatsapp_history.*","CONCAT(whatsapp_api_config.gateway_name,' : ',whatsapp_api_config.phone_number) AS send_as");
        $join=array("whatsapp_api_config"=>"whatsapp_history.gateway_id=whatsapp_api_config.id,left");
        $offset = ($page-1)*$rows;
        $result = array();
        $info=$this->basic->get_data('whatsapp_history', $where, $select, $join, $limit=$rows, $start=$offset, $order_by=$order_by_str, $group_by='', $num_rows=0);
        $total_rows_array=$this->basic->count_row($table="whatsapp_history", $where, $count="whatsapp_history.id", $join);
        $total_result=$total_rows_array[0]['total_rows'];
        echo convert_to_grid_data($info, $total_result);
    }
    ...
    //     $data['body']="my_whatsapp/send_whatsapp/send_whatsapp";
    ...
    //     $data['page_title'] = 'Send WhatsApp';
    //     $this->_viewcontroller($data);
    // }


    // public function whatsapp_send_action()
    // {
    //     if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    //         redirect('home/access_forbidden', 'location');
    //     }
    ...
    //     $config_id= strip_tags(trim($this->input->post('from_whatsapp', true)));
    ...
    //     }
    //     /***Set Whatsapp Credential ***/
    //     $this->whatsapp_manager->set_credentioal($config_id);
    //     $sent_number=array();
    ...
    //         $this->whatsapp_manager->send_whatsapp($message_replaced, $phone_number);
    //     }

    //     /***** Send whatsapp which numbers are not in the contact list ******/
    ...
    //         if ($numbers!='')
    //         {
    //             $this->whatsapp_manager->send_whatsapp($message, $numbers);
    //         }
    //     }
    // }
    ...
    public function credit_display_crud($value, $row)
    {
        ...
        $credit="-";
        $this->whatsapp_manager->set_credentioal($row->id,$this->user_id);
        if($gateway_name=="plivo") $credit=$this->whatsapp_manager->get_plivo_balance();
        if($gateway_name=="clickatell") $credit=$this->whatsapp_manager->get_clickatell_balance();
        if($gateway_name=="clickatell-platform") $credit=$this->whatsapp_manager->get_clickatell_platform_balance();
        if($gateway_name=="nexmo") $credit=$this->whatsapp_manager->get_nexmo_balance();
        if($gateway_name=="africastalking.com") $credit=$this->whatsapp_manager->africastalking_whatsapp_balance();
        if($gateway_name=="infobip.com") $credit=$this->whatsapp_manager->infobip_balance_check();
        if($gateway_name=="Shreeweb") $credit=$this->whatsapp_manager->get_shreeweb_balance();
    ...
    public function api_field_crud($value, $row)
    {
        ...
        $str.='<option value="">'.$this->lang->line('WhatsApp API').'</option>';
        for ($i=0;$i<count($whatsapp_api_config);$i++) {
            if ($whatsapp_api_config[$i]['id']==$value) {
                $str.='<option selected="selected" value="'.$whatsapp_api_config[$i]['id'].'">'.$whatsapp_api_config[$i]['gateway_name'].": ".$whatsapp_api_config[$i]['phone_number'].'</option>';
            } else {
                $str.='<option value="'.$whatsapp_api_config[$i]['id'].'">'.$whatsapp_api_config[$i]['gateway_name'].": ".$whatsapp_api_config[$i]['phone_number'].'</option>';
            }
        }
        $str.='</select>';
        return $str;
    ...
    public function api_display_crud($value, $row)
    {
        ...
        $join=array("whatsapp_api_config"=>"whatsapp_api_config.id=birthday_reminder.api_id,left");
        $whatsapp_api_config=$this->basic->get_data('birthday_reminder', $where, $select=array("whatsapp_api_config.gateway_name", "whatsapp_api_config.phone_number"), $join);
        return $whatsapp_api_config[0]['gateway_name']." : ".$whatsapp_api_config[0]['phone_number'];
    }

    public function insert_user_id_whatsapp_template($post_array, $primary_key)
    {
        $user_id=$this->user_id;
    ...
    public function insert_user_id_whatsapp_api($post_array, $primary_key)
    {
        $user_id=$this->user_id;
        $update_data=array('user_id'=>$user_id);
        $where=array("id"=>$primary_key);
        $this->basic->update_data("whatsapp_api_config", $where, $update_data);
    }
    ...
    public function whatsapp_template_field($value, $row)
    {
        $where=array("where"=>array('user_id'=>$this->user_id));
    ...
    public function generateSerialWhatsAppAPI()
    {
        if ($this->session->userdata('WhatsAppAPILastSerial') == '') {
            $this->session->set_userdata('WhatsAppAPILastSerial', 0);
            $this->session->set_userdata('WhatsAppAPILastPage', 1);
            $lastSerial = 0;
        } else {
            $lastSerial = $this->session->userdata('WhatsAppAPILastSerial');
        }

        $lastSerial++;
        $page = $this->input->post('page');
        if ($page != '') {
            $this->session->set_userdata('WhatsAppAPILastPage', $page);
        } else {
            $this->session->set_userdata('WhatsAppAPILastPage', 1);
        }
        $this->session->set_userdata('WhatsAppAPILastSerial', $lastSerial);
        return $lastSerial;
    }

    public function generateSerialWhatsAppTemplate()
    {
        if ($this->session->userdata('WhatsAppTemplateLastSerial') == '') {
            $this->session->set_userdata('WhatsAppTemplateLastSerial', 0);
            $this->session->set_userdata('WhatsAppTemplateLastPage', 1);
            $lastSerial = 0;
        } else {
            $lastSerial = $this->session->userdata('WhatsAppTemplateLastSerial');
        }

        $lastSerial++;
        $page = $this->input->post('page');
        if ($page != '') {
            $this->session->set_userdata('WhatsAppTemplateLastPage', $page);
        } else {
            $this->session->set_userdata('WhatsAppTemplateLastPage', 1);
        }
        $this->session->set_userdata('WhatsAppTemplateLastSerial', $lastSerial);
        return $lastSerial;
    }


    public function generateSerialBirhdayWhatsApp()
    {
        if ($this->session->userdata('birthdayWhatsAppLastSerial') == '') {
            $this->session->set_userdata('birthdayWhatsAppLastSerial', 0);
            $this->session->set_userdata('birthdayWhatsAppLastPage', 1);
            $lastSerial = 0;
        } else {
            $lastSerial = $this->session->userdata('birthdayWhatsAppLastSerial');
        }

        $lastSerial++;
        $page = $this->input->post('page');
        if ($page != '') {
            $this->session->set_userdata('birthdayWhatsAppLastPage', $page);
        } else {
            $this->session->set_userdata('birthdayWhatsAppLastPage', 1);
        }
        $this->session->set_userdata('birthdayWhatsAppLastSerial', $lastSerial);
        return $lastSerial;
    }
